...
def get_all_ksized_partitions(seq, partition_size):
    """
    - seq: sequence to partition
    - k: size of each partition
    """
    # get size-k partitions
    def _rec(seq, k):
        assert len(seq) % k == 0 # equal sized
        if len(seq)==k:
            return [[seq]]
        ...
        all_partitions = []
        # n choose k subsets
        for subset_indices in itertools.combinations(indices, k):
            subset_indices = set(subset_indices)
            subset1 = [[seq[i] for i in subset_indices]]
            subset2 = [seq[i] for i in indices if i not in subset_indices]

            # (n-k) choose k subsets
            subset2_partitions = _rec(subset2, k)

            # aggregate
            partitions = [p+subset1 for p in subset2_partitions]
            all_partitions.extend(partitions)

        return all_partitions

    # sort+set to remove k! partition duplicates
    all_partitions = _rec(seq, partition_size)
    all_partitions = [tuple(map(tuple, sorted(p))) for p in all_partitions]
...
def update_ax(ax, title=None, xlabel=None, ylabel=None,
              ...
              ticks=True, grid=True, grid_kws=None, legend_bbox=None):
